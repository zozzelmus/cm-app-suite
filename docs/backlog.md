...
- ~~**Wire authorization at the API edge.**~~ **PARTIAL — F9.** API has `AddJwtBearer` + `FallbackPolicy = RequireAuthenticatedUser`; BFF YARP route has `AuthorizationPolicy = "default"` and forwards the access token. **Remaining for F10:** per-endpoint `IAuthorizationService.HasPermissionAsync(...)` checks + JIT user-mirror provisioning + audience validation.
