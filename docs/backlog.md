...
- ~~**Wire request-derived tenant resolution.**~~ **CLOSED by F9.** `TenantContextMiddleware` reads the `tenant_id` claim from the validated JWT and 401s if absent. RLS now has a real tenant value to scope on.
- ~~**Wire authorization at the API edge.**~~ **CLOSED by F9 + F10.** API: `AddJwtBearer` + `FallbackPolicy = RequireAuthenticatedUser`. BFF: `AuthorizationPolicy = "default"` on YARP, access-token forwarding. F10 added `IConductAuthorization.HasPermissionAsync(...)` w/ LOB ancestor inheritance, JIT User mirror w/ inactive→403, and an imperative `case.create` check on `POST /api/cases`. **Still deferred:** audience validation on JwtBearer (separate slice; track inline in `AuthSetup.cs`).
